...
class IMU:
    def __init__(self):
        ...
        # Initialize the Adafruit MPU6050 sensor
        i2c = board.I2C()
    ...
    def imuUpdate(self):
        ...
        ax = self.kalman_filter_AX.kalman(accel_data[0] - self.Error_value_accel_data['x'])
        ay = self.kalman_filter_AY.kalman(accel_data[1] - self.Error_value_accel_data['y'])
        az = self.kalman_filter_AZ.kalman(accel_data[2] - self.Error_value_accel_data['z'])
        gx = self.kalman_filter_GX.kalman(gyro_data[0] - self.Error_value_gyro_data['x'])
        gy = self.kalman_filter_GY.kalman(gyro_data[1] - self.Error_value_gyro_data['y'])
        gz = self.kalman_filter_GZ.kalman(gyro_data[2] - self.Error_value_gyro_data['z'])
